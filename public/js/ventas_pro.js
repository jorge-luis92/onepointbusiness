...
$(document).on("click", "#btnpesos", function(){
  ...
  let id_pr = document.getElementById("tipo").value;
  $("#id_produc").val(id_pr);
  let isChecked = document.getElementById('cbox2').checked;
if(isChecked){
...
$(document).on("click", "#btngramoss", function(){
  $("#cantidad_gram").val("");
  let isChecked = document.getElementById('cbox1').checked;
if(isChecked){
...
let sihay= revisarStock(id, gram, sinocheck);
}else {
...
let checpesos= 0;
    ...
    $.ajax({
         ...
         success : function(respuesta) {
           const prueba = JSON.parse(respuesta);
           let alma=0, almados=0, pres=0;
           let vals = prueba.unidad_venta;
           if(checpesos == 0){
           pres = parseInt(prueba.precio);}
           else {
             pres = parseInt(prueba.promocion);
           }
           if(vals == "Gramos"){
    ...
    let sihay= revisarStock(cuack, alma, checpesos);
    }else {
           alert("El producto ya se encuentra insertado");
    }
         }
     ...
     function revisarStock(id, gram, checando) {
            let valor= id;
            let gramitos= parseInt(gram);
            let checsino= checando;
           $.ajax({
                type: "get",
                url: "/revisarSt/"+valor,
                datatype: 'json',
                      ...
                      // data:  {id:id},
                       success : function(respuesta) {
                         ...
                         if(cuvs == "Gramos"){
                          ...
                          gana = gana+((cant_a_venders/1000)*prueba.precio_may);
                          gananciaspues = totaltemp - gana;
                          otrogano=gananciaspues.toFixed(2);
                           totalstemp=totaltemp.toFixed(2);
                           $("#totalV").html(totalstemp);
                         }else {
                           totaltemp=totaltemp+((cant_a_venders/1000)*prueba.promocion);
                         gana = gana+((cant_a_venders/1000)*prueba.precio_may);
                         gananciaspues = totaltemp - gana;
                         otrogano=gananciaspues.toFixed(2);
                          totalstemp=totaltemp.toFixed(2);
                         ...
                         if(checsino == 0){
                           totaltemp=totaltemp+(cant_a_venders*prueba.precio);
                             gana = gana+(cant_a_venders*prueba.precio_may);
                             gananciaspues = totaltemp - gana;
                             otrogano=gananciaspues.toFixed(2);
                           totalstemp=totaltemp.toFixed(2);
                               $("#totalV").html(totalstemp);
                         }else {
                           totaltemp=totaltemp+(cant_a_venders*prueba.promocion);
                             gana = gana+(cant_a_venders*prueba.precio_may);
                             gananciaspues = totaltemp - gana;
                             otrogano=gananciaspues.toFixed(2);
                           totalstemp=totaltemp.toFixed(2);
                       ...
                       if(checsino == 0){
                        let fila = `<tr>
                                ...
                                <td id="nombrepro" scope="row">${prueba.nombre}</td>
                                  <td id="preciov" scope="row">$&nbsp;${prueba.precio}&nbsp;${prueba.unidad}</td>
                                  <td id="gramitos"  scope="row">${gramitos} ${prueba.unidad_venta} </td>
                                  <td id="tipodegr"  scope="row">${totalstemp} </td>
                                  <td scope="row"><button id="dd" class="btn-danger"> <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-backspace-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" d="M15.683 3a2 2 0 0 0-2-2h-7.08a2 2 0 0 0-1.519.698L.241 7.35a1 1 0 0 0 0 1.302l4.843 5.65A2 2 0 0 0 6.603 15h7.08a2 2 0 0 0 2-2V3zM5.829 5.854a.5.5 0 1 1 .707-.708l2.147 2.147 2.146-2.147a.5.5 0 1 1 .707.708L9.39 8l2.146 2.146a.5.5 0 0 1-.707.708L8.683 8.707l-2.147 2.147a.5.5 0 0 1-.707-.708L7.976 8 5.829 5.854z"/>
                                            ...
                                            let myProduct = { id: prueba.id, nombre: prueba.nombre, tipo: prueba.tipo, totaltempo: totalstemp, gramos: gramitos, ganancias: otrogano};
                                            ...
                                            if(cuv == "Gramos"){
                                             total=total+((cant_a_vender/1000)*prueba.precio);
                                              totales=total.toFixed(2);
                                              $("#totalV").html(totales);
                                          document.getElementById("id_producs").value=totales;
                                            }else {
                                                total=total+(cant_a_vender*prueba.precio);
                                            totales=total.toFixed(2);
                                                  $("#totalV").html(totales);
                                                document.getElementById("id_producs").value=totales;
                                            }}
                                              ...
                                              let fila = `<tr>
                                                      <td id= "id_p"scope="row">${prueba.id}</td>
                                                      <td id="nombrepro" scope="row">${prueba.nombre}</td>
                                                        <td id="preciov" scope="row">$&nbsp;${prueba.promocion}&nbsp;${prueba.unidad} promocion</td>
                                                        <td id="gramitos"  scope="row">${gramitos} ${prueba.unidad_venta} </td>
                                                        <td id="tipodegr"  scope="row">${totalstemp} </td>
                                                        <td scope="row"><button id="dd" class="btn-danger"> <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-backspace-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                                          <path fill-rule="evenodd" d="M15.683 3a2 2 0 0 0-2-2h-7.08a2 2 0 0 0-1.519.698L.241 7.35a1 1 0 0 0 0 1.302l4.843 5.65A2 2 0 0 0 6.603 15h7.08a2 2 0 0 0 2-2V3zM5.829 5.854a.5.5 0 1 1 .707-.708l2.147 2.147 2.146-2.147a.5.5 0 1 1 .707.708L9.39 8l2.146 2.146a.5.5 0 0 1-.707.708L8.683 8.707l-2.147 2.147a.5.5 0 0 1-.707-.708L7.976 8 5.829 5.854z"/>
                                                                  ...
                                                                  $("#productos_insertados").append(fila);
                                                                  let nuevopro=[prueba.id, prueba.nombre, prueba.promocion, gramitos];
                                                                  let myProduct = { id: prueba.id, nombre: prueba.nombre, tipo: prueba.tipo, totaltempo: totalstemp, gramos: gramitos, ganancias: otrogano};
                                                                  probando_objects.push(myProduct);
                                                                  agregado_a.push(nuevopro);
                                                                  posiciones.push(prueba.id);
                                                                  ...
                                                                  let cant_a_vender = parseInt(gramitos);
                                                                  if(cuv == "Gramos"){
                                                                   total=total+((cant_a_vender/1000)*prueba.promocion);
                                                                    totales=total.toFixed(2);
                                                                    $("#totalV").html(totales);
                                                                document.getElementById("id_producs").value=totales;
                                                                  }else {
                                                                      total=total+(cant_a_vender*prueba.promocion);
                                                                  totales=total.toFixed(2);
                                                                        $("#totalV").html(totales);
                                                                      document.getElementById("id_producs").value=totales;
                                                                  }
