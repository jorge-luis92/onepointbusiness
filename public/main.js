...
$(document).on("click", "#btngramos", function(){
  ...
  $("#id_produc").val(id_pr);
  console.log("si está dando id?: "+id_pr);
    $(".modal-title").text("Inserte Cantidad");
    $('#gramosa').modal('show');
...
$('#gramosa').modal('hide');
    ...
    console.log(id_delp+"ay vamos"+ gram);
    console.log("entrando");
       let id = $("#tipo").val();
       console.log(id);
         ...
         $.ajax({
                ...
                success : function(respuesta) {
                  ...
                  let val = prueba.stock;
                    console.log("imprimiendo datos stock: ");
                  console.log(val);
                 if (val > 0) {
                   $.ajax({
                    //   headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
